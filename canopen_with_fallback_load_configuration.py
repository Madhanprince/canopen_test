...
class A5Control:
    ...
    def _load_configuration(self) -> None:
        """
        Wrapper around RemoteNode.load_configuration().

        That method was only added to python-canopen in 2.4.0. If you're
        pinned to canopen 2.3.0 (no load_configuration attribute), this
        falls back to a manual reimplementation of what it does:
        push PDO mapping from the OD, then download all other writable
        OD entries via SDO.
        """
        if hasattr(self.remote, "load_configuration"):
            self.remote.load_configuration()
            return

        self.log.warning(
            "canopen<2.4.0 detected (RemoteNode.load_configuration missing) "
            "— using manual fallback"
        )

        # 1. Push PDO mapping config from the object dictionary
        self.remote.pdo.read(from_od=True)
        self.remote.pdo.save()

        # 2. Download all other writable, non-PDO OD entries via SDO
        from canopen.objectdictionary import ODArray, ODRecord, ODVariable

        def _download(index, subindex, name, value):
            try:
                if subindex is None:
                    self.remote.sdo[index].raw = value
                else:
                    self.remote.sdo[index][subindex].raw = value
            except Exception:
                self.log.warning(
                    f"Failed to download OD entry {name} "
                    f"(index={index}, subindex={subindex})",
                    exc_info=True,
                )

        for obj in self.remote.object_dictionary.values():
            if 0x1400 <= obj.index < 0x1C00:
                continue  # PDO-related, already handled above
            if isinstance(obj, (ODRecord, ODArray)):
                for subobj in obj.values():
                    if isinstance(subobj, ODVariable) and subobj.writable and subobj.value is not None:
                        _download(subobj.index, subobj.subindex, subobj.name, subobj.value)
            elif isinstance(obj, ODVariable) and obj.writable and obj.value is not None:
                _download(obj.index, None, obj.name, obj.value)
